 ...
 * @Author @Corbanator
 * 
 * This class is designed to take strings and turn them into actual objects that are presets, which will then be used to generate actual objects in the generator class.
 * It is called almost exclusively by the Generator class.
 */
public class PresetLoader {

    /**
     * This method takes a json file with an object that has one attribute, "rooms", that is an array of rooms, and uses other methods in the class to load in an array of RoomPresets.
     * 
     * @param toLoad
     * @return ArrayList<RoomPreset>
     */
    public static ArrayList<RoomPreset> loadRoomPresets(String toLoad) {
        //uses parseArray to find the array with the key "rooms" and load in each room in that array
    ...
    /** 
     * This method takes in a string that is a roomPreset object in json format and parses it into an actual RoomPreset object
     * 
     * @param toLoad
     * @return RoomPreset
     */
    private static RoomPreset loadRoomPreset(String toLoad) {
        ...
        //TrimQuotes gets rid of the quotes that will be in the string array that is returned so that those are not printed
        String[] descriptions = Parser.trimQuotes(Parser.parseArray("descriptions", toLoad));

        //Uses loadInventoryPresets to load in an inventory of interactablePresets, but only if that array is not empty
        ...
        //This section does essentially the same thing for normal interactables instead of special description interactables, 
        //but it is slightly more complicated because this field is optional, so it must check if it exists.
    ...
    /** 
     * Similar to loadRoomPresets, this loads in an entire array, or "inventory", of interactablePresets
     * @param toLoad
    ...
    /** 
     * This method parses for all of the attributes of an InteractablePreset and makes one based on a string that is a jsonObject.
     * @param toLoad
     * @return InteractablePreset
     */
    public static InteractablePreset loadInteractablePreset(String toLoad) {
        ...
        //Parse other various attributes, such as size, weight, and whether it can be picked up.
        ...
        //loads in abilities. These don't actually do anything yet, but it leaves the door open for more complex interactables going forward.
        ...
        //creates an InteractablePreset that will be returned or added to, depending on its type.
        ...
        //if the interactablePreset is a container or weapon, it must parse additional attributes.
    ...
    /** 
     * this method is called only by loadInteractablePreset to load the remaining information about a ContainerPreset.
     * @param preset
     * @param toLoad
    ...
    /** 
     * This method loads in an NPCPreset from a string that is a JSONObject, including all of its attributes and whatever it has in its inventory.
     * @param toLoad
     * @return NPCPreset
     */
    public static NPCPreset loadNpcPreset(String toLoad){
        ...
        //A description will be chosen out of the array at random when the preset is generated.
        String descriptions[] = Parser.trimQuotes(Parser.parseArray("descriptions", toLoad));

        //Parse various ranges of attributes, where a number in the range will be selected when the NPC is actually created from the preset
        ...
        //Loads in the inventory by obtaining an array of strings to then load in as InteractablePresets
