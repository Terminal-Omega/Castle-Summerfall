...
public class Floor {
    ...
    /**
     * gets a door based on coordinates and a direction, or throws an exception if
     * that door doesn't exist. multiple sets of inputs can obtain the same door, as
     * each door is technically connected to two rooms.
     *
     * @param xCoord
     * @param yCoord
     * @param direction
    ...
    public Door getDoor(int xCoord, int yCoord, Direction direction) throws ThingNotFoundException {
        ...
        case NORTH:
            /**
             * Both North and West rooms are a bit tricky, as they are stored in a room
             * different from the one with the coordinates given. As a result, it is
             * required that I check if this room exists before attempting to get a door
             * from it.
             */
    ...
    /**
     * This method returns the description of a certain room based on location. It
     * is required to be in the floor class because it must include doors and
     * enemies, both of which are inaccessible from the actual Room itself.
     *
     * @param xCoord
     * @param yCoord
     * @return String
    ...
    public String getDescription(int xCoord, int yCoord) {
        ...
        /**
         * If there are stairs in the room, then add a special portion to the
         * description explaining that there is either a boss or that the boss is
         * defeated.
         */
        ...
        // check each direction for a door, then add it to a string. Has some slight
        // grammatical issues for two doors in particular.
        int j = 0;
    ...
    /**
     * Returns an NPC based on name and location. The format for the parameters is
     * designed to make it easy to use on the frontend. This is because the NPC name
     * passed in by the player can then be passed directly to this method.
     *
     * @param name
     * @param xCoord
     * @param yCoord
